class MailRequest < ActiveRecord::Base
	...
	def check_validation
		...
		errors.add(:weight, "Cannot be nil") if weight == nil	
		errors.add(:volume, "Cannot be nil") if volume == nil
		errors.add(:weight, "- Weight must be greater than zero") if weight == nil or weight<=0
		errors.add(:volume, "- Volume must be greater than zero") if volume == nil or volume<=0
	end
end
